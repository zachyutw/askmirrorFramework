        ...
        <div className={[ 'itemSelection', className ].join(' ')} {...rest}>
            {options.map((option, index) => (
                <WithFieldInputItemP
                    ...
                    name='item'
                    {...option}
                    render={RenderProp && <RenderProp {...option} onChange={onChange} />}
...
const InputItemP = (props) => {
    const { input = {}, text, style, className, ref, to, innerRef, replace, icon, render, children, ...rest } = props;
    ...
    return (
        <div
            ...
            ref={ref}
            {...rest}
            className={[ className, 'item', actived ? 'active' : '' ].join(' ')}
